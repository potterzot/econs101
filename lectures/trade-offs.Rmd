...
<iframe width="560" height="315" src="https://www.youtube.com/embed/GY4yJoSSMZM" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
...
```{r trade-offs, echo=FALSE, out.width='75%'}
...
* **Average product** - The average amount of **production** per unit of input. 

* **Marginal product** - The additional **production** from increasing a single unit of input.
...
```{r marginal_consumption, out.width = "75%"}
d1 <- data.frame(t = 0:16)
d1$hw1 <- 40 + log(d1$t+1)/max(log(d1$t)) * 55
d1$hw2 <- 20 + log(d1$t+1, base = exp(2))/max(log(d1$t), base = exp(2)) * 400
